...
### A/B Testing Prompts — Choosing Between Two Versions

Sometimes you have two candidate versions of a prompt and you genuinely do not know which performs better on real production data. Your test suite passes for both. The changelog is written for both. How do you decide which to deploy?

This is where **A/B testing** comes in. Instead of deploying one version fully, you split your production traffic between the two versions — for example, 50% of applications go through prompt v1.1 and 50% go through v1.2 — and measure the outcomes over a defined period.

**For the Step 2 scoring prompt in the screening pipeline:**

```
Week 1: 
- 200 applications scored by v1.1 (original weighting)
- 200 applications scored by v1.2 (increased experience weighting)

Measure after Week 1:
- Shortlist rate: did v1.2 change the proportion of candidates shortlisted?
- Score distribution: did v1.2 produce a wider or narrower score range?
- Human reviewer agreement: when borderline cases went to human review,
  did reviewers agree more often with v1.1 or v1.2 recommendations?
```

After the measurement period, the version with better outcomes on the metrics that matter is deployed fully. The losing version is archived in version control.

**Why A/B testing matters:** your test suite uses fixed, known inputs. Real production data is messy, varied, and often contains inputs your test cases never anticipated. A/B testing on real data catches differences that controlled testing misses.

**The minimum A/B test for the screening pipeline:** run both versions on at least 100 real applications each, measure shortlist rate and human reviewer agreement rate, and choose the version whose outputs the human reviewers agree with more often.
...
**Which failure modes to address first:**
Not all failure modes carry equal risk. When building a new pipeline, prioritise in this order:

1. **Prompt injection** — can be exploited maliciously and cause immediate harm. Add the mitigation before the first production deployment, not after
2. **Cascading errors** — a single wrong extraction can corrupt every downstream step. Add range checks between steps before going live
3. **Context window overflow** — silent and hard to debug once it occurs. Set token monitoring from day one
4. **Hallucinated structure** — adds anti-hallucination instructions to your extraction prompts before first deployment
5. **Prompt drift** — less urgent on day one but must be monitored from the first week of production
6. **Silent success** — the slowest-moving failure mode. Start the weekly sampling audit in the first week, even if the sample is small
...
- Consider using dedicated LLMOps platforms (like LangSmith, Promptfoo, or Weights & Biases) which provide enterprise-grade tools for prompt versioning, testing, and tracking out of the box.
