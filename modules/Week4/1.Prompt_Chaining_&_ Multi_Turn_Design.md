...
### A Note on Parallel Chaining

The chains described above are **sequential** — each step waits for the previous one to finish before starting. But some pipelines can run steps in parallel when the steps do not depend on each other's output.

For the job screening pipeline, once extraction (Step 1) is complete, you could run scoring (Step 2) and a duplicate-application check simultaneously — both only need the extracted data, not each other's results. Only after both finish does the email draft (Step 3) begin.

**Think of it like a kitchen preparing a meal.** The chef does not finish the starter, then begin the main, then begin the dessert in strict sequence. While the starter is being plated, the main is already in the oven and the dessert is chilling. Steps that do not depend on each other run at the same time.

Parallel chaining reduces total processing time significantly on high-volume pipelines. For 400 applications, sequential processing of all four steps might take 40 minutes. Running independent steps in parallel could cut this to 20 minutes or less. The design principle is simple: if Step B does not need Step A's output, they can run at the same time.
