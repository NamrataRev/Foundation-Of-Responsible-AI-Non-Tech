...
**Exponential backoff — waiting between retries:**

Not all failures are caused by the model getting the answer wrong. Sometimes the API call itself fails — the server is overloaded, a token or request Rate Limit has been hit (HTTP 429 errors), or there is a temporary network issue. In these cases, retrying immediately makes the problem worse — you are adding more requests to an already overloaded system.

**Exponential backoff** means waiting a progressively longer time between each retry attempt:

```
Attempt 1 fails → wait 1 second → Attempt 2
Attempt 2 fails → wait 2 seconds → Attempt 3
Attempt 3 fails → wait 4 seconds → Attempt 4
Attempt 4 fails → escalate to fallback
```

Each wait time doubles. This gives the server time to recover and prevents your pipeline from hammering the API with rapid repeated requests.

**For the screening pipeline:** if Step 2 returns a server error (not a wrong answer — an actual API error), the retry should wait before trying again. Most AI provider SDKs include built-in exponential backoff — always check whether your library handles this automatically before building your own.
