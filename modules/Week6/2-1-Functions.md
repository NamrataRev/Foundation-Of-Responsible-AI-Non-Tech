...
**Early return — using `return` as a guard clause:**

A common pattern is to return early when a condition is not met, avoiding deeply nested `if` blocks:

```python
def calculate_discount(price, discount_pct):
    if discount_pct < 0 or discount_pct > 100:
        return None   # invalid — exit immediately

    discount = price * (discount_pct / 100)
    return price - discount

print(calculate_discount(200, 20))    # 160.0
print(calculate_discount(200, 110))   # None — invalid discount
```

The first `return` acts like a door check — if the input is invalid, exit right away. The rest of the function only runs when the input is valid. This keeps the main logic clean and easy to read.
...
**One rule you will hit quickly — defaults must come last:**

```python
def describe(age=18, name):   # SyntaxError
    pass

def describe(name, age=18):   # correct — non-defaults first, defaults after
    pass
```

Python needs to know which arguments are required and which are optional before it can figure out what goes where. Required parameters always come before optional ones. Break this rule and Python tells you immediately with a `SyntaxError`.
...
- Putting a default parameter before a non-default one — `def f(age=18, name)` is a `SyntaxError`; defaults must always come after required parameters
...
- `return` sends a value back and exits immediately; no `return` means the function gives back `None`. Use early returns as guard clauses to handle invalid input before the main logic runs.
- Arguments can be positional, keyword, or have defaults; defaults must always come after required parameters — `def f(name, age=18)`, not `def f(age=18, name)`.
- `*args`/`**kwargs` collect an unknown number of extra arguments.
