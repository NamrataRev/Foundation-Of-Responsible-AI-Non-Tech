...
![Pinecone files panel showing multiple uploaded markdown files each covering one topic — clean file structure prevents bad chunking](../images/Fileupload.png)
...
You already saw this in the RAG Failure and Recovery topic — the screenshot showed an answer generated with the Assistant instructions box completely empty. The answer still had some citations because Pinecone adds them automatically — but uncited sentences from general training slipped through without any source reference.

Here is a clearer example — the same question asked with no Assistant instructions. The answer is detailed and well-formatted but has **zero citations** anywhere:

![Pinecone playground showing detailed answer about while loops with no citation numbers anywhere — Assistant instructions box completely empty on the right panel](../images/w10_03_no_citations.png)

Notice two things: the Assistant instructions box on the right shows only the placeholder text — completely empty. And the answer contains "Google Colab" — which was never in the uploaded notes. It came from general training. With no citation instruction, the model blended retrieved content and general knowledge with nothing to distinguish which was which.
